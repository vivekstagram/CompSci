...
public class ThirteensBoard extends Board {
	...
	public boolean anotherPlayIsPossible() {
		...
		return findPairSum13(cIndexes).size() >= 2 || containsKing(cIndexes);
	}
	...
	private boolean containsKing(List<Integer> selectedCards) {
		...
		for (Integer kingIndex : selectedCards) 
		{
			int k = kingIndex.intValue();
	...
	private List<Integer> findKing(List<Integer> selectedCards) {
		List<Integer> kingIndex = new ArrayList<Integer>();
	...
	private boolean playKingIfPossible()
	{
		...
		{
			super.replaceSelectedCards(findKing(super.cardIndexes()));
			return true;
		}
		else
		{
